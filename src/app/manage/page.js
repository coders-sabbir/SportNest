...
import { 
  ...
  MapPin, Users, AlertTriangle, X, ArrowRight, Loader2
} from "lucide-react";
...
export default function ManageFacilitiesPage() {
  ...
  // ১. ডাটাবেস থেকে রিয়েল ডাটা ফেচ করা
  useEffect(() => {
    const fetchFacilities = async () => {
      try {
        const response = await axios.get("https://sport-nest-server-a4sz.vercel.app/facilities");
  ...
  // ২. ডিলিট করার রিকোয়েস্ট (পরবর্তীতে ব্যাকএন্ডে রুট অ্যাড করতে হবে)
  const confirmDelete = async () => {
    if (!facilityToDelete) return;

    setIsDeleting(true);
    try {
      // API call to delete (তুমি পরে index.js এ এই রুটটা বানাবে)
      await axios.delete(`https://sport-nest-server-a4sz.vercel.app/facilities/${facilityToDelete._id}`);
      
      setFacilities(facilities.filter(f => f._id !== facilityToDelete._id));
      alert(`${facilityToDelete.name} has been removed successfully!`);
      closeDeleteModal();
    } catch (error) {
      console.error("Error deleting facility:", error);
      alert("Failed to delete. Have you added the DELETE route in backend yet?");
    } finally {
      setIsDeleting(false);
    }
  };
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        {isLoading ? (
          <div className="flex justify-center items-center h-64">
             <Loader2 size={40} className="text-[#A3FF12] animate-spin" />
          </div>
        ) : (
          <>
            {/* Desktop View*/}
              ...
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-white/5">
                  {filteredFacilities.map((facility) => (
                    <tr key={facility._id} className="hover:bg-white/[0.02] transition-colors group">
                        ...
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            ...
                            <div className="flex items-center gap-2 text-xs text-gray-400">
                              <span className="text-[#A3FF12]">{facility.facility_type}</span>
                      ...
                      <td className="py-5 px-6">
                        <div className="flex flex-col gap-1 text-sm">
                          <div className="text-white font-semibold">${facility.price_per_hour} <span className="text-gray-500 font-normal">/hr</span></div>
                      ...
                      <td className="py-5 px-6">
                        <div className="flex flex-col items-start gap-2">
                          ...
                          <div className="flex items-center gap-1 text-sm font-semibold text-white">
                            <Star size={14} className="text-yellow-400 fill-current" /> {facility.rating || "5.0"} <span className="text-gray-500 font-normal text-xs">({facility.booking_count || 0})</span>
                          </div>
                        </div>
                      </td>
                      <td className="py-5 px-6 text-right">
                        <div className="flex items-center justify-end gap-2">
                          <Link href={`/facilities/${facility._id}`} className="p-2.5 bg-white/5 border border-white/10 rounded-xl hover:bg-white/10 text-gray-400 hover:text-white transition-all outline-none group/btn">
                            <Eye size={16} />
                          </Link>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 lg:hidden mb-12">
              {filteredFacilities.map((facility) => (
                <div key={facility._id} className="bg-[#111827]/60 backdrop-blur-xl border border-white/10 rounded-2xl p-5 relative overflow-hidden flex flex-col">
                  ...
                  <div className="flex justify-between items-start mb-4">
                    ...
                    <div className="flex items-center gap-1 text-sm font-semibold text-white">
                      <Star size={14} className="text-yellow-400 fill-current" /> {facility.rating || "5.0"}
                    </div>
                  </div>
                  ...
                  <div className="flex gap-4 mb-4">
                    ...
                    <div className="flex flex-col justify-center">
                      <span className="text-[#A3FF12] text-xs font-bold uppercase mb-1">{facility.facility_type}</span>
                  ...
                  <div className="grid grid-cols-2 gap-2 mb-5">
                    <div className="bg-white/5 p-2.5 rounded-xl border border-white/5 text-center">
                      <div className="text-white font-bold">${facility.price_per_hour}</div>
                      <div className="text-xs text-gray-500">Per Hour</div>
                    </div>
                    <div className="bg-white/5 p-2.5 rounded-xl border border-white/5 text-center">
                      <div className="text-white font-bold">{facility.booking_count || 0}</div>
          ...
          <div className="relative w-full max-w-md bg-[#111827] border border-red-500/30 rounded-3xl p-8 shadow-[0_0_50px_rgba(239,68,68,0.15)] transform transition-all">
            ...
            <div className="flex flex-col items-center text-center">
              ...
              <p className="text-gray-400 text-sm mb-8">
                Are you sure you want to remove <strong className="text-white">{facilityToDelete?.name}</strong>? This action cannot be undone.
              </p>
              ...
              <div className="flex w-full gap-3">
                <button 
                  onClick={closeDeleteModal}
                  disabled={isDeleting}
                  className="flex-1 bg-white/5 border border-white/10 text-white font-semibold py-3.5 rounded-xl hover:bg-white/10 transition-colors outline-none disabled:opacity-50"
                >
                  Cancel
                </button>
                <button 
                  ...
                  disabled={isDeleting}
                  className="flex-1 bg-red-500 text-white font-bold py-3.5 rounded-xl hover:bg-red-600 shadow-[0_0_20px_rgba(239,68,68,0.3)] transition-all outline-none flex items-center justify-center gap-2 disabled:opacity-50"
                >
                  {isDeleting ? <><Loader2 size={16} className="animate-spin" /> Removing...</> : "Yes, Remove"}
                </button>
              </div>
            </div>
          </div>
